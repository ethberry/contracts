...
export function shouldBurn(factory: () => Promise<any>, options: IERC721EnumOptions = {}) {
  ...
  describe("burn", function () {
    it("should reset token royalty info", async function () {
      ...
      await expect(tx).to.emit(contractInstance, "Transfer").withArgs(owner.address, ZeroAddress, 1);
