...
abstract contract ERC721ConsecutiveEnumerable is ERC721, IERC721Enumerable, IERC2309 {
  ...
  function tokenByIndex(uint256 index) public view virtual override returns (uint256) {
    require(index < ERC721ConsecutiveEnumerable.totalSupply(), "ERC721Enumerable: global index out of bounds");
  ...
  function _mint(address to, uint256 tokenId) internal virtual override {
    require(Address.isContract(address(this)), "ERC721ConsecutiveEnumerable: can't mint during construction");
  ...
  function _beforeTokenTransfer(
    ...
    if (batchSize > 1) {
      require(!Address.isContract(address(this)), "batch minting is restricted to constructor");
    } else {
      if (from == address(0)) {
   ...
   * @dev See {IERC721Enumerable-tokenOfOwnerByIndex}.
   */
  function _ownerTokenByIndex(address owner, uint256 index) private view returns (uint256) {
  ...
  function _ownerIndexByToken(uint256 tokenId) private view returns (uint256) {
  ...
  // private
  function _mintConsecutive(address to, uint96 batchSize) internal returns (uint96) {
    ...
    if (batchSize > 0) {
      ...
      require(ERC721.balanceOf(to) == 0, "each account can batch mint once");
      ...
      while (first < last) {
        if (last - first > 5000) {
          emit ConsecutiveTransfer(first, first + 4999, address(0), to);
          first = first + 5000;
        } else {
