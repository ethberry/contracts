...
abstract contract StateHash is IStateHash {
  ...
  function _afterTokenTransfer(address from, address, uint256 tokenId) internal virtual {
    if (from == address(0)) {
  ...
  function _afterReceiveERC721(address, uint256 _tokenId, address _childContract, uint256 _childTokenId)
    internal
    virtual
  {
    uint256 rootId = _localRootId(_tokenId);
