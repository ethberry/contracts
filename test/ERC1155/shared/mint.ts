...
export function shouldMint() {
  describe("mint", function () {
    it("should fail for wrong role", async function () {
      const tx = this.erc1155Instance.connect(this.receiver).mint(this.receiver.address, 1, amount, "0x");
    ...
    it("should mint to wallet", async function () {
      const tx = this.erc1155Instance.mint(this.receiver.address, tokenId, amount, "0x");
      await expect(tx)
    ...
    it("should fail to mint to non receiver", async function () {
      const tx = this.erc1155Instance.mint(this.erc1155NonReceiverInstance.address, tokenId, amount, "0x");
