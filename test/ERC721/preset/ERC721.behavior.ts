...
import { ERC721ACBCES } from "../../../typechain-types";
//import { ERC721TestInterface } from "./Interfaces/ERC721TestInterface";
import { ERC721TestInterface } from "./Interfaces/ERC721TestInterface";
//import { ERC721ACBCES as  ERC721TestInterface} from "./Interfaces/ERC721ACBCES";

//export {ERC721TestInterface as ERC721ACBCES} from "./Interfaces/ERC721ACBCES";
...
export function shouldBehaveLikeERC721 (token: ERC721TestInterface, owner: any, receiver: any) {
//export function shouldBehaveLikeERC721 <Type> (token: Type, owner: any, receiver: any) {
/*    shouldSupportInterfaces([
    ...
    describe("constructor", function () {
        it.only("Should set the right roles to deployer", async function () {
    ...
    describe("mint", function () {
        ...
        it.only("should fail for wrong role", async function () {
            const tx = token.connect(receiver).mint(receiver.address);
            await expect(tx).to.be.revertedWith(
              `AccessControl: account ${receiver.address.toLowerCase()} is missing role ${MINTER_ROLE}`,
            );
        });
      
        it.only("should mint to wallet", async function () {
            const tx = token.mint(owner.address);
            await expect(tx).to.emit(token, "Transfer").withArgs(ethers.constants.AddressZero, owner.address, 0);
 ...
 /*       it.only("should fail for wrong role", async function () {
            console.log("----- 1 ---------");
            const tx = await erc721Instance.connect(receiver).mint(receiver.address);
            console.log(tx); 
            console.log("----- 2 ---------");
        });*/
...
//            console.log("----- 0 1 ---------");
          ...
          console.log(tx);
          console.log("----- 2 ---------");
          //await expect(tx).to.emit(this.erc721Instance, "Transfer").withArgs(ethers.constants.AddressZero, owner.address, 0);
    
         // const balance = await this.erc721Instance.balanceOf(owner.address);
         // expect(balance).to.equal(1);
        });*/
    });
