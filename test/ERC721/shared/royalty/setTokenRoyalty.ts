...
export function shouldSetTokenRoyalty(roles = false) {
  describe("setTokenRoyalty", function () {
    it("should set token royalty", async function () {
      ...
      const tx = this.erc721Instance.setTokenRoyalty(0, this.receiver.address, royalty);
      await expect(tx).to.emit(this.erc721Instance, "TokenRoyaltyInfo").withArgs(0, this.receiver.address, royalty);
    });
    ...
    it("should fail: invalid parameters", async function () {
      ...
      const tx = this.erc721Instance.setTokenRoyalty(0, ethers.constants.AddressZero, royalty);
    ...
    it("should fail: not admin", async function () {
      ...
      const tx = this.erc721Instance.connect(this.receiver).setTokenRoyalty(0, this.receiver.address, royalty);
