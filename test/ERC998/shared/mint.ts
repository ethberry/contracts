...
export function shouldMint() {
  describe("mint", function () {
    it("should fail for wrong role", async function () {
      const tx = this.erc998Instance.connect(this.receiver).mint(this.receiver.address);
    ...
    it("should mint to wallet", async function () {
      const tx = this.erc998Instance.mint(this.owner.address);
    ...
    it("should mint to non receiver", async function () {
      const tx = this.erc998Instance.mint(this.erc721NonReceiverInstance.address);
      await expect(tx)
    ...
    it("should mint to receiver", async function () {
      const tx = this.erc998Instance.mint(this.erc721ReceiverInstance.address);
      await expect(tx)
        .to.emit(this.erc998Instance, "Transfer")
