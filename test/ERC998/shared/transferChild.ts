...
export function shouldTransferChild() {
  describe("transferChild", function () {
    it("should transfer token owned by another token to wallet", async function () {
      ...
      const tx2 = this.erc998Instance.transferChild(1, this.receiver.address, this.erc721Instance.address, 0);
      await expect(tx2)
    ...
    it("should transfer token owned by another token to the receiver contract", async function () {
      ...
      const tx2 = this.erc998Instance.transferChild(1, this.erc721ReceiverInstance.address, this.erc721Instance.address, 0);
      await expect(tx2)
        .to.emit(this.erc998Instance, "TransferChild")
    ...
    it("should transfer token owned by another token to the non receiver contract", async function () {
      ...
      const tx2 = this.erc998Instance.transferChild(1, this.erc721NonReceiverInstance.address, this.erc721Instance.address, 0);
      await expect(tx2)
        .to.emit(this.erc998Instance, "TransferChild")
        .withArgs(1, this.erc721NonReceiverInstance.address, this.erc721Instance.address, 0);
    });
    ...
    it("should not transfer token which is not owned", async function () {
      ...
      const tx = this.erc998Instance.transferChild(1, this.receiver.address, this.erc721Instance.address, 0);
    ...
    it("should transfer 998 token owned by another token to the wallet", async function () {
      ...
      const tx2 = this.erc998Instance.transferChild(2, this.receiver.address, this.erc998Instance.address, 1);
      await expect(tx2)
        .to.emit(this.erc998Instance, "TransferChild")
